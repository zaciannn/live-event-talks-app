## 2026-06-22T05:46:22Z
You are Worker 1 (worker_m1). Your working directory is `/mnt/c/Users/Lenovo/Documents/agy-cli-projects/bq-releases-notes/.agents/worker_m1/`.
Your mission is to implement Milestone 1: Backend API in the workspace root `/mnt/c/Users/Lenovo/Documents/agy-cli-projects/bq-releases-notes/`.

1. Write `requirements.txt` with:
```text
Flask>=3.0.0
requests>=2.31.0
```
...
class FeedCache:
    def __init__(self, cache_file=CACHE_FILE, duration=CACHE_DURATION_SECONDS):
...
3. Ensure a `static` folder is created at the root level so `index.html` can be served. For now, write a tiny placeholder `static/index.html` file (e.g., `<h1>BigQuery Release Notes</h1>`) so the Flask app starts up successfully and serves the static folder.
4. Run verification checks:
   - Install dependencies.
   - Start the Flask app using a background task or run it on a separate terminal.
   - Query `/api/releases` using `curl` and verify it returns a valid JSON array and custom HTTP headers.
